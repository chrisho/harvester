...
Hybrid Pods resource usage calculation refers to the resource usage of both VM and non-VM Pods deployed in the same namespace.

## Proposal

---

Increase the resource limit of ResourceQuota based on the specifications of the target VM before migration, and restore (decrease) the resource limit to its original level after the migration is complete.

### User Stories

**VM Migration**

Users need to migrate VMs in Harvester, but the migration fails due to exhausted resources. When VM migration is in Pending status, Harvester should automatically scale up ResourceQuota to meet the resource requirements for VM migration. After the VM migration is completed, Harvester should automatically scale down the quota.
...
For a Harvester node undergoing maintenance, if the Force parameter is provided, Harvester should properly shut down single-replica VMs and migrate multiple-replica VMs to other nodes. Since single-replica VMs have already been shut down, they will not be counted towards resource scaling.
...
The Migration Controller monitors the status of the migration process. When the migration status is Pending, it automatically scales up the ResourceQuota to meet the resource requirements of the VM migration. After the migration is complete, it automatically scales down the quota.
...
The yey is vm_name with a value of the corresponding VM's Pod resources limits. When Migration is in Pending, Harvester should automatically expand the ResourceQuota to meet the resource requirements for the VM migration. Once the VM migration is complete, Harvester should automatically scale down and delete the key.
...
**For the same Namespace, if a non-VM Pod is in Pending status while Migration is also in Pending status, the non-VM Pod may compete for resources and cause the migration to fail. We do not recommend deploying VMs and non-VM Pods in the same Namespace.**
